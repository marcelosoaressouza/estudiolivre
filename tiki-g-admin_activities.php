...
// $Header: /cvsroot/tikiwiki/tiki/tiki-g-admin_activities.php,v 1.10.2.9 2007/11/28 23:36:54 kerrnel22 Exp $
...
if (isset($_REQUEST['save_act'])) {
	...
	// If a role is selected from the drop-down list, then use it,
	// otherwise if the dropdown is "add new" then use the data in
	// the text input field.  If a user is selected from the drop-down
	// AND a value is in the input field, then the drop-down will
	// override the input field.
	if ($_REQUEST['userole'] != 'add new') {
		$rid = $_REQUEST['userole'];
	} else if (!empty($_REQUEST['rolename'])) {
		$vars = array(
			'name' => $_REQUEST['rolename'],
			'description' => ''
		);
