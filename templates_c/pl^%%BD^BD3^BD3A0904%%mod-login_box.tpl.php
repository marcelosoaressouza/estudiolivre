...
<?php if (! $this->_tpl_vars['user']): ?>
	<?php $this->_tag_stack[] = array('tikimodule', array('title' => 'Logowanie','name' => 'login')); $_block_repeat=true;smarty_block_tikimodule($this->_tag_stack[count($this->_tag_stack)-1][1], null, $this, $_block_repeat);while ($_block_repeat) { ob_start(); ?>
		<form id="uLoginBox" action="tiki-login.php" method="post">
			<?php if ($this->_tpl_vars['isIE']): ?>Usuári@: <?php endif; ?><input class="<?php if (! $this->_tpl_vars['isIE']): ?>uText<?php endif; ?>" type="text" name="user" id="login-user" size="12" <?php if ($this->_tpl_vars['isIE']): ?>style="width:60%"<?php endif; ?> <?php if (! $this->_tpl_vars['isIE']): ?>value="użytkownik" onFocus="if(this.value=='użytkownik')this.value=''"<?php endif; ?>/>
			<?php if ($this->_tpl_vars['isIE']): ?>Senha: <?php endif; ?><input class="<?php if (! $this->_tpl_vars['isIE']): ?>uText<?php endif; ?>" type="<?php if ($this->_tpl_vars['isIE']): ?>password<?php else: ?>text<?php endif; ?>" name="pass" id="login-pass" size="10"	<?php if ($this->_tpl_vars['isIE']): ?>style="width:68%"<?php endif; ?> <?php if (! $this->_tpl_vars['isIE']): ?>value="hasło" onFocus="if(this.value=='hasło')this.value='';this.type='password'"<?php endif; ?>/>
			<?php if ($this->_tpl_vars['isIE']): ?><center><input type="submit" value="zaloguj się" name="login"/></center><?php else: ?><?php $this->_tag_stack[] = array('tooltip', array('text' => "Clique aqui ou aperte <i>Enter</i> para efetuar o login")); $_block_repeat=true;smarty_block_tooltip($this->_tag_stack[count($this->_tag_stack)-1][1], null, $this, $_block_repeat);while ($_block_repeat) { ob_start(); ?><input type="image" name="login" src="styles/estudiolivre/iLogin.png" /><?php $_block_content = ob_get_contents(); ob_end_clean(); $_block_repeat=false;echo smarty_block_tooltip($this->_tag_stack[count($this->_tag_stack)-1][1], $_block_content, $this, $_block_repeat); }  array_pop($this->_tag_stack); ?><?php endif; ?>
...
<?php else: ?>
	<?php $this->_tag_stack[] = array('tikimodule', array('title' => "Użytkownik</a>",'name' => 'user')); $_block_repeat=true;smarty_block_tikimodule($this->_tag_stack[count($this->_tag_stack)-1][1], null, $this, $_block_repeat);while ($_block_repeat) { ob_start(); ?>
	<div id="userMod">		
