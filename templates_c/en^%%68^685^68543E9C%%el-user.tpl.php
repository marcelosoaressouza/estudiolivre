<?php /* Smarty version 2.6.18, created on 2011-04-04 17:27:20
      ...
      <div id="uContact" class="uContactInfoCont left">
        <?php if ($this->_tpl_vars['permission']): ?>
        	<?php $this->_tag_stack[] = array('tooltip', array('text' => "Clique para modificar o seu <b>nome</b>")); $_block_repeat=true;smarty_block_tooltip($this->_tag_stack[count($this->_tag_stack)-1][1], null, $this, $_block_repeat);while ($_block_repeat) { ob_start(); ?>
	        	<?php echo smarty_function_ajax_input(array('permission' => $this->_tpl_vars['permission'],'id' => 'realName','class' => 'uContactItem','value' => $this->_tpl_vars['realName'],'default' => 'Full Name','display' => 'block'), $this);?>
...
<?php endif; ?>" display="block"><?php echo ((is_array($_tmp=((is_array($_tmp=$this->_tpl_vars['site'])) ? $this->_run_mod_handler('replace', true, $_tmp, 'http://', '') : smarty_modifier_replace($_tmp, 'http://', '')))) ? $this->_run_mod_handler('truncate', true, $_tmp, 30, "(...)", true) : smarty_modifier_truncate($_tmp, 30, "(...)", true)); ?>
        ...
        <?php if ($this->_tpl_vars['permission']): ?>
        	<?php $this->_tag_stack[] = array('tooltip', array('text' => "Clique para modificar a sua <b>localização</b>")); $_block_repeat=true;smarty_block_tooltip($this->_tag_stack[count($this->_tag_stack)-1][1], null, $this, $_block_repeat);while ($_block_repeat) { ob_start(); ?>
		        <?php echo smarty_function_ajax_input(array('permission' => $this->_tpl_vars['permission'],'id' => 'local','class' => 'uContactItem','value' => $this->_tpl_vars['local'],'default' => 'Location','display' => 'inline'), $this);?>
...
"/>
			    	   	<?php $_block_content = ob_get_contents(); ob_end_clean(); $_block_repeat=false;echo smarty_block_tooltip($this->_tag_stack[count($this->_tag_stack)-1][1], $_block_content, $this, $_block_repeat); }  array_pop($this->_tag_stack); ?>
			    	<?php else: ?>
	    		    	<img id="ajax-uLicence" alt="(Choose a default license)"/>
	    		    <?php endif; ?>
	    		    </span>
	        <?php else: ?>
		        <?php if ($this->_tpl_vars['licenca']): ?>
...
"/><?php $_block_content = ob_get_contents(); ob_end_clean(); $_block_repeat=false;echo smarty_block_tooltip($this->_tag_stack[count($this->_tag_stack)-1][1], $_block_content, $this, $_block_repeat); }  array_pop($this->_tag_stack); ?>
		    	<?php else: ?>
		        	(This user has no default license)
		        <?php endif; ?>
		    <?php endif; ?>
        	</span>
        </span>
...
&sort_mode=lastModif_desc&days=0">Recent Contributions</a></span>
...
">Tags desse usuário</a></span>
        <br />
        <span class="uContactItem uLittle">Member since <?php echo ((is_array($_tmp=$this->_tpl_vars['userinfo']['registrationDate'])) ? $this->_run_mod_handler('date_format', true, $_tmp, "%d/%m/%Y") : smarty_modifier_date_format($_tmp, "%d/%m/%Y")); ?>
</span>
      </div>  
    </div>
 ...
 ?>
	   			...
	   			<?php if ($this->_tpl_vars['permission']): ?>
					<div id="ajax-elIce" style="display:none;">
						...
						<input type="button" onClick="xajax_set_mount_point(document.getElementById('ajax-livePoint').value,document.getElementById('ajax-livePass').value)" value="Submit"/><input type="button" onClick="flip('ajax-elIce')" value="Cancel"/>
						<br/>
...
unset($_smarty_tpl_vars);
 ?></div>
	  <?php else: ?>
	  	<?php if ($this->_tpl_vars['permission']): ?>
		  	<p> You still don't have any files on the Gallery. <a href="el-gallery_upload.php">Share</a> your work!</p>
	  	<?php else: ?>
	  		<p> This user has not yet published any works on the Gallery.</p>
	  	<?php endif; ?>
	  <?php endif; ?>
      </div>
    </div>
...
&show_comments=1#comments">(<?php echo $this->_tpl_vars['post']['commentsCount']; ?>
		...
		<?php else: ?>
		  	<?php if ($this->_tpl_vars['permission']): ?>
			  	<p> You don't have any blog posts yet. <a href="tiki-edit_blog.php">Create</a> a blog or check out the <a href="tiki-list_blogs.php">list</a> of the existing ones.</p>
		  	<?php else: ?>
...
unset($_smarty_tpl_vars);
 ?>
	      <?php else: ?>
	      	<?php if ($this->_tpl_vars['permission']): ?>
	      		<p> You don't have a personal wiki page yet. <a href="tiki-editpage.php?page=<?php echo $this->_tpl_vars['pageName']; ?>
">Create</a> your personal wiki page now!</p>
	      	<?php else: ?>
	      		<p> This user doesn't have a personal wiki page </p>
	      	<?php endif; ?>
	      <?php endif; ?>
        </div>
    </div>
