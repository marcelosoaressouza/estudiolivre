   	...
   	<div class="comMainTitle">
		    <?php if ($this->_tpl_vars['forum_mode'] == 'y'): ?>
			    ...
			    <?php elseif ($this->_tpl_vars['parent_com']): ?>
			    	Beantwoordt het aangeduide bericht
		    	<?php else: ?>
		    		Post new message
		    	<?php endif; ?>
	    	<?php else: ?>
		    	<?php if ($this->_tpl_vars['comments_threadId'] > 0): ?>
		    		Bewerking van opmerking
		    	<?php elseif ($this->_tpl_vars['parent_com']): ?>
		    		Lever commentaar op het aangeduide bericht
		    	<?php else: ?>
...
" />
    	<?php endfor; endif; ?>
  		...
  		<?php if ($this->_tpl_vars['parent_coms']): ?>
				Beantwoordt bovenliggend bericht
		<?php else: ?>
				<?php if ($this->_tpl_vars['forum_mode'] == 'y'): ?>
					Beantwoorden
				<?php endif; ?>
...
" />
		...
		<?php endif; ?>
		<div id="comButtons">
			<input type="submit" name="comments_previewComment" value="voorbeeld tonen"/>
			<input type="submit" name="comments_postComment" value="voeg toe"/>
			
			<?php if ($this->_tpl_vars['forum_mode'] == 'y'): ?>
				<input type="button" name="comments_cancelComment" value="annuleren" onclick="hide('<?php echo $this->_tpl_vars['postclass']; ?>
');"/>
		    <?php endif; ?>
	    </div>
	...
	Gebruik [http://www.foo.com] of [http://www.foo.com|omschrijving] voor koppelingen.<br />
	HTML-labels zijn niet toegestaan in berichten.<br />
