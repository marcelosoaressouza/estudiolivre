...
"Attention" => "Atención",
"Apache has the right to write in your file tree, which enables the installation, removal or \nupgrade of packages. When you are done with those operations, think to fix those permissions back to a safe state (by using \n\"./fixperms.sh fix\" for example)." => "Apache tiene el derecho a escribir en su sistema de archivos, que permite la instalación, borrado o actualización de paquetes. Cuando termine esas operaciones, piense en arreglar esos permission de nuevo a un estado seguro (usando ./fixperms.sh fix, p/ej)",
"To install, remove or upgrade packages you need to give the apache user the right\nto write files in your web tree (you can use \"./fixperms.sh open\" to set it up). After installation you need to remove that\npermission (using \"./fixperms.sh fix\")." => "Para instalar, borrar o actualizar paquetes necesita darle al usuario del apache el derecho t¡a escribir archivos en su arbol web (puede usar \"./fixperms.sh open\" para dejarlo asi)). Luego de la instalacion debe borrare esos permisos (usando \"./fixperms.sh fix\").",
