...
class StructLib extends TikiLib {
  ...
  */
	...
	function get_toc($page_ref_id,$order='asc',$showdesc=false,$numbering=true,$numberPrefix='',$type='plain',$maxdepth=0) {
		$structure_tree = $this->build_subtree_toc($page_ref_id,false,$order,$numberPrefix);
		return $this->fetch_toc($structure_tree,$showdesc,$numbering,$type,$maxdepth);
	}

	function fetch_toc($structure_tree,$showdesc,$numbering,$type='plain',$maxdepth=0, $cur_depth=0) {
		global $smarty;
		$ret='';
		if ($structure_tree != '') {
			if (($maxdepth <= 0) || ($cur_depth < $maxdepth)) {
				...
				foreach($structure_tree as $leaf) {
					...
					if(isset($leaf["sub"]) && is_array($leaf["sub"])) {
						$ret.=$this->fetch_toc($leaf["sub"],$showdesc,$numbering,$type, $maxdepth, $cur_depth+1);
					} else {
						$ret.="</li>";
					}
				}
