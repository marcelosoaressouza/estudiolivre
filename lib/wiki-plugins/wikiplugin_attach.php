<?php
// $Header: /cvsroot/tikiwiki/tiki/lib/wiki-plugins/wikiplugin_attach.php,v 1.6.4.18 2006/09/08 08:51:30 sylvieg Exp $
      ...
      if(isset($image) and $image )
      {
        ...
        if (isset($icon)) {
          ...
          if (isset($mimeextensions["$ext"]) and (is_file("img/icn/".$mimeextensions["$ext"].".gif"))) {
            ...
            $string = strtolower(substr($atts['
                                              data'][$n]['filename'], strlen($atts['data'][$n]['filename'])-3));
            if (is_file("img/icn/".$string.".gif"))
              $link.= '<img src="img/icn/'.$string.'.gif" border="0" />&nbsp; ';
            else
              $link.= '
                      <img src="img/icn/else.gif" border="0" />&nbsp; ';
          }
        }
        if (isset($showdesc) && !empty($atts['
                                             data'][$n]['comment'])) {
