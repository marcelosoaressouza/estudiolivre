<?php
// $Header: /cvsroot/tikiwiki/tiki/tiki-admin_system.php,v 1.15.2.13 2007/06/26 17:31:15 mose Exp $
...
function du($path, $begin=null) {
  ...
  while($file = readdir($all)) {
    if(is_dir($path.'/'.$file) and $file <> ".." and $file <> "." and $file <> "CVS" and !in_array($file,$multis)) {
      $du = du($path.'/'.$file);
      $total+= $du['total'];
      $cant+= $du['cant'];
      unset($file);
...
function erase_dir_content($path) {
  ...
  if($dir = opendir($path)) {
    while(false !== ($file = readdir($dir))) {
      ...
      if(in_array($file,$multis)) continue;
...
function cache_templates($path,$newlang) {
  ...
  if($dir = opendir($path)) {
    while(false !== ($file = readdir($dir))) {
      ...
      if(substr($file,0,1) == "." or $file == 'CVS' or in_array($file,$multis)) continue;
