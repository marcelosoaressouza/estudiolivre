...
// $Header: /cvsroot/tikiwiki/tiki/tiki-adminusers.php,v 1.34.2.41 2008/01/30 22:37:47 sylvieg Exp $
...
function batchImportUsers() {
  ...
  if($fields[0]!="login" && $fields[0]!="password" && $fields[0]!="email" && $fields[0]!="realname" && $fields[0]!="groups") {
    $smarty->assign('msg', tra("The file does not have the required header:")." login, password, email, groups");
    $smarty->display("error.tpl");
    die;
  }
  ...
  foreach($userrecs as $u) {
    ...
    elseif($userlib->user_exists($u['login'])and(!isset($_REQUEST['overwrite']))) {
      ...
      if(!$userlib->user_exists($u['login'])) {
        $userlib->add_user($u['login'], $u['password'], $u['email'], '', NULL, $u['realname']);
        $logslib->add_log('users',sprintf(tra("Created account %s <%s> for %s."),$u["login"], $u["email"], $u["realname"]));
      }
  ...
  if((!$_REQUEST["name"]) and(is_uploaded_file($_FILES['csvlist']['tmp_name']))) {
    ...
    if($_REQUEST["pass"] != $_REQUEST["pass2"]) {
      ...
      elseif(!preg_match($patterns['login'],$_REQUEST['name'])) {
        ...
        if($userlib->add_user($_REQUEST["name"], $_REQUEST["pass"], $_REQUEST["email"], '', NULL, $_REQUEST['realname'])) {
          $tikifeedback[] = array('num'=>0,'mes'=>sprintf(tra("New %s created with %s %s (%s)."),tra("user"),tra("username"),$_REQUEST["name"], $_REQUEST["realname"]));
          $cookietab = '1';
...
for($i=0; $i<$arraylen; $i++) {
  ...
  $users['data'][$i]['realName'] = $userlib->get_user_name($users['data'][$i]['user']);
}
...
if(isset($_REQUEST["user"]) and $_REQUEST["user"]) {
  ...
  $userinfo['realName'] = $userlib->get_user_name($userinfo['login']);

  if(isset($_POST["edituser"]) and isset($_POST['name']) and isset($_POST['email']) and isset($_POST['realname'])) {
    //var_dump($_POST);die;
    if($_POST['name']) {
    ...
    if($userinfo['realName'] != $_POST['realname']) {
      if($userlib->change_user_name($userinfo['login'],$_POST['realname'])) {
        $tikifeedback[] = array('num'=>0,'mes'=>sprintf(tra("%s changed from %s to %s"),tra("real name"),$userinfo['realName'],$_POST["realname"]));
        $logslib->add_log('adminusers','','changed realName for '.$_POST['name'].' from '.$userinfo['realName'].' to '.$_POST["realname"]);
        $userinfo['realName'] = $_POST['realname'];
      }

      else {
        $tikifeedback[] = array('num'=>1,'mes'=>sprintf(tra("Impossible to change %s from %s to %s"),tra("real name"),$userinfo['realName'],$_POST["realname"]));
      }
    }
