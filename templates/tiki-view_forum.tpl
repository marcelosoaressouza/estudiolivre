{* $Header: /cvsroot/tikiwiki/tiki/templates/tiki-view_forum.tpl,v 1.60.2.21 2007/07/09 18:42:41 nkoth Exp $ *}
...
{if $user and $feature_user_watches eq 'y'}
	{if $user_watching_forum eq 'n'}
		<a href="tiki-view_forum.php?forumId={$forumId}&amp;watch_event=forum_post_topic&amp;watch_object={$forumId}&amp;watch_action=add" title='{tr}monitor this forum{/tr}'><img border='0' alt='{tr}monitor this forum{/tr}' src='img/icons/icon_watch.png' /></a>
	{else}
		<a href="tiki-view_forum.php?forumId={$forumId}&amp;watch_event=forum_post_topic&amp;watch_object={$forumId}&amp;watch_action=remove" title='{tr}stop monitoring this forum{/tr}'><img border='0' alt='{tr}stop monitoring this forum{/tr}' src='img/icons/icon_unwatch.png' /></a>
	{/if}
{/if}
{if $user and $feature_user_watches eq 'y'}
...
<tr>
  ...
  <a {if $comments_coms[ix].is_marked}class="forumnameread"{else}class="forumname"{/if}  href="tiki-view_forum_thread.php?comments_parentId={$comments_coms[ix].threadId}{if $comments_threshold}&amp;topics_threshold={$comments_threshold}{/if}{if $comments_offset or $smarty.section.ix.index}&amp;topics_offset={math equation="x + y" x=$comments_offset y=$smarty.section.ix.index}{/if}{if $comments_sort_mode ne 'commentDate_desc'}&amp;topics_sort_mode={$comments_sort_mode}{/if}{if $topics_find}&amp;topics_find={$comments_find}{/if}&amp;forumId={$forum_info.forumId}">{$comments_coms[ix].title}</a>
  {if $forum_info.topic_summary eq 'y'}
