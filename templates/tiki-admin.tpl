...
{if in_array($smarty.get.page, array("features", "general", "login", "wiki", "gal", "fgal", "cms", "polls", "search", "blogs", "forums", "faqs", "trackers", "webmail", "rss", "directory", "userfiles", "maps", "metatags", "wikiatt","score", "community", "siteid", "calendar","intertiki","gmap", "i18n", "category", "module", "theme", "textarea"))}
...
{/if}
{if $feature_version_checks eq 'y' and $tiki_needs_upgrade eq 'y'}
...
{* Hiding links to tiki-backup.php for fresh install in Tiki 1.9.8 until we fix or remove.
...
{if $tiki_p_admin eq 'y' and $feature_debug_console eq 'y'}
  ...
  ({tr}Click and scroll up to the top of the page{/tr})
{/if}
