<div class="cbox">
  ...
  <div class="cbox-data">
	...
	<tr>
	  ...
	  <td class="form">
	{if $feature_help eq 'y'}<a href="{$helpurl}Search" target="tikihelp" class="tikihelp" title="{tr}Full Text Search{/tr}">{/if}
...
{if $feature_help eq 'y'}<a href="{$helpurl}Search+Stats" target="tikihelp" class="tikihelp" title="{tr}SearchStats{/tr}">{/if}
	...
	<tr>
          <td class="form">
        {if $feature_help eq 'y'}<a href="{$helpurl}Referrer+Search+Highlighting" target="tikihelp" class="tikihelp" 
title="{tr}Referer Search Highlighting{/tr}">{/if}
...
<tr><td><a href="tiki-admin.php?page=search&refresh_tracker_index_now=y" class="link" title="{tr}Refresh trackers search index now{/tr}">
                        {tr}Refresh tracker search index now{/tr}</a></td><td></td></tr>
{/if}
	<tr>
          <td class="heading" colspan="2">{tr}Performance issues{/tr}</td>
        </tr><tr>
	<td class="form">
	{if $feature_help eq 'y'}<a href="{$helpurl}Search" target="tikihelp" class="tikihelp" title="{tr}Search may show forbidden results. Much better performance though.{/tr}">{/if}
