{* $Id: tiki-plugin_trackerlist.tpl,v 1.9.4.30 2007/12/09 14:54:55 kerrnel22 Exp $ *}
...
{else}
<a href="{$smarty.server.PHP_SELF}?{if $page}page={$page|escape:url}&amp;refresh=1&amp;{/if}tr_initial={$initials[ini]}{if $tr_sort_mode}&amp;tr_sort_mode={$tr_sort_mode}{/if}&amp;tr_offset=0" class="prevnext">{$initials[ini]}</a> . 
{/if}
{/section}
<a href="{$smarty.server.PHP_SELF}?{if $page}page={$page|escape:url}&amp;refresh=1&amp;{/if}tr_initial={if $tr_sort_mode}&amp;tr_sort_mode={$tr_sort_mode}{/if}&amp;tr_offset=0" class="prevnext">{tr}All{/tr}</a>
</div>
{/if}
...
<table class="normal">
{if $showfieldname eq 'y'}
<tr>
{if $checkbox}<td class="heading">{$checkbox.title}</td>{/if}
{if ($showstatus ne 'n') and ($tracker_info.showStatus eq 'y' or ($tracker_info.showStatusAdminOnly eq 'y' and ($perms_tiki_p_admin eq 'y' or $perms.tiki_p_admin_trackers eq 'y')))}
...
{if $perms.tiki_p_admin eq 'y' or $perms.tiki_p_tracker_view_ratings eq 'y'}
<td class="heading auto"{if $perms.tiki_p_admin eq 'y' or $perms.tiki_p_tracker_vote_ratings eq 'y'} colspan="2"{/if}>
<a class="tableheading" href="{$smarty.server.PHP_SELF}?{if $page}page={$page|escape:url}&amp;refresh=1&amp;{/if}tr_sort_mode=f_{if $tr_sort_mode eq 'f_'|cat:$ix.fieldId|cat:'_asc'}{$ix.fieldId}_desc{else}{$ix.fieldId}_asc{/if}{if $tr_offset}&amp;tr_offset={$tr_offset}{/if}{if $tr_initial}&amp;tr_initial={$tr_initial}{/if}">{$ix.name|default:"&nbsp;"}</a></td>
{/if}
{else}
...
<td class="heading"><a class="tableheading" href="{$smarty.server.PHP_SELF}?{if $page}page={$page|escape:url}&amp;refresh=1&amp;{/if}tr_sort_mode={if 
...
{if ($showstatus ne 'n') and ($tracker_info.showStatus eq 'y' or ($tracker_info.showStatusAdminOnly eq 'y' and ($perms.tiki_p_admin eq 'y' or $perms.tiki_p_admin_trackers eq 'y')))}<td class="auto" style="width:20px;">
...
{if $items[user].field_values[ix].value}{$items[user].field_values[ix].value|tiki_short_datetime}{else}&nbsp;{/if}
...
{if $perms.tiki_p_admin eq 'y' or $perms.tiki_p_view_trackers eq 'y' or $perms.tiki_p_modify_tracker_items eq 'y' or $perms.tiki_p_comment_tracker_items eq 'y' or $items[user].field_values[ix].linkId}</a>{/if}
</td>
{else}
...
{elseif $items[user].field_values[ix].type eq 's' and $items[user].field_values[ix].name eq "Rating"}
{if $perms.tiki_p_admin eq 'y' or $perms.tiki_p_tracker_view_ratings eq 'y'}
<td class="auto">
<b title="{tr}Rating{/tr}: {$items[user].field_values[ix].value|default:"-"}, {tr}Number of voices{/tr}: {$items[user].field_values[ix].numvotes|default:"-"}, {tr}Average{/tr}: {$items[user].field_values[ix].voteavg|default:"-"}">&nbsp;{$items[user].field_values[ix].value|default:"-"}&nbsp;</b>
</td>
{if $perms.tiki_p_admin eq 'y' or $perms.tiki_p_tracker_vote_ratings eq 'y'}
<td class="auto" nowrap="nowrap">
<span class="button2">
<b class="linkbut{if $items[user].my_rate eq NULL} highlight{/if}">-</b>
...
{else}
<a href="{$smarty.server.PHP_SELF}?{if $page}page={$page|escape:url}&amp;refresh=1&amp;{/if}trackerId={$items[user].trackerId}&amp;itemId={$items[user].itemId}&amp;fieldId={$items[user].field_values[ix].fieldId}&amp;rate_{$items[user].trackerId}={$items[user].field_values[ix].options_array[i]}" class="linkbut">{$items[user].field_values[ix].options_array[i]}</a>
{/if}
{/section}
</span>
</td>
...
{if $tr_prev_offset >= 0}
[<a class="prevnext" href="{$smarty.server.PHP_SELF}?{if $page}page={$page|escape:url}&amp;refresh=1&amp;{/if}tr_offset={$tr_prev_offset}{
...
{if $tr_next_offset >= 0}
&nbsp;[<a class="prevnext" href="{$smarty.server.PHP_SELF}?{if $page}page={$page|escape:url}&amp;refresh=1&amp;{/if}tr_offset={$tr_next_offset}{
	if $tr_initial}&amp;tr_initial={$tr_initial}{/if}{
	if $tr_sort_mode}&amp;tr_sort_mode={$tr_sort_mode}{/if}"
>{tr}next{/tr}</a>]
{/if}
...
<a class="prevnext" href="{$smarty.server.PHP_SELF}?{if $page}page={$page|escape:url}&amp;refresh=1&amp;{/if}tr_offset={$selector_offset}{
	if $tr_initial}&amp;tr_initial={$tr_initial}{/if}{
	if $tr_sort_mode}&amp;tr_sort_mode={$tr_sort_mode}{/if}">
