...
{*    <input maxlength="20" size="10" type="text" name="wiki_forum" value="{$wiki_forum|escape}"/> *}
  ...
  <div class="cbox">
    ...
    <tr><td class="form"><a href="tiki-admin.php?page=textarea" title="{tr}Moved to:{/tr} {tr}Text area{/tr}"><del style="text-decoration: line-through; color: #999; background-color: #fff">{tr}Anonymous editors must input anti-bot code{/tr}</del></a>:</td><td><input type="checkbox" disabled="disabled" name="feature_antibot" {if $feature_antibot eq 'y'}checked="checked"{/if}/></td></tr>
