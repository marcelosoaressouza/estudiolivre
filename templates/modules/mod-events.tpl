...
$thedate = mktime(23,59,59,intval($mon),intval($day),intval($year));
...
{php}
		...
		for ($i=0;$i<6;$i++) {
			...
			for ($j=0;$j<7;$j++) {
				...
				$newval="";
				unset($tmp);
				$tmp = $items[mktime(0, 0, 0, intval($mon), intval($val), intval($year))];
				if (isset($tmp)) if (is_array($tmp)) {
					unset($items[mktime(0, 0, 0, intval($mon), intval($val), intval($year))]);
					foreach ($tmp as $xx) {
