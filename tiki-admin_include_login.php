<?php
// $Header: /cvsroot/tikiwiki/tiki/tiki-admin_include_login.php,v 1.19.2.17 2007/09/05 08:32:09 nkoth Exp $
...
if (isset($_REQUEST["loginprefs"])) {
    ...
    if (isset($_REQUEST["auth_method"])) {
    	if ($_REQUEST["auth_method"] == "intertiki") {
			$tikilib->set_preference('auth_method', 'tiki');
			$tikilib->set_preference('feature_intertiki', 'y');
    	} else {
			$tikilib->set_preference('auth_method', $_REQUEST['auth_method']);
			$tikilib->set_preference('feature_intertiki', 'n');
		}
...
if (isset($_REQUEST["auth_pear"])) {
    ...
    if (isset($_REQUEST["auth_type"])) {
	$tikilib->set_preference("auth_type", $_REQUEST["auth_type"]);

	$smarty->assign('auth_type', $_REQUEST["auth_type"]);
    }

    if (isset($_REQUEST["auth_imap_pop3_basedsn"])) {
	$tikilib->set_preference("auth_imap_pop3_basedsn", $_REQUEST["auth_imap_pop3_basedsn"]);

	$smarty->assign('auth_imap_pop3_basedsn', $_REQUEST["auth_imap_pop3_basedsn"]);
    }
