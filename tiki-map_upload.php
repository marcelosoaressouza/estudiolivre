...
// $Header: /cvsroot/tikiwiki/tiki/tiki-map_upload.php,v 1.8.4.18 2007/09/13 19:55:50 ohertel Exp $
...
if (!is_dir($map_path)) {
  $smarty->assign('msg', tra("Please create a directory to hold your map files:")." ".$map_path);
	$smarty->display('error.tpl');
	die;
}
...
if (isset($_REQUEST["action"]) && isset($_REQUEST["indexfile"]) 
  ...
  if ($_REQUEST["action"]=="createindex") {
      ...
      if ($indexfile && $filestoindex && is_file($gdaltindex)) {
        ...
        $filestoindex=$filestoindex.$DSEP.basename($_REQUEST["filestoindex"]);
